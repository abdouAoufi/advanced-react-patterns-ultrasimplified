...
import userCostumStyle from "./usage.css";
...
const Clap = ({ children, onClap, style: userStyles = {}, className }) => {
  ...
  const compnentJustMounted = useRef(true); // this will be true when component just mounted!
  ...
  const classNames = [styled.clap, className].join(" ").trim("");
  return (
...
const ClapIcon = ({ style: userStyle = {}, className }) => {
  ...
  const classNames = [styled.icon, isClicked ? styled.checked : "", className]
    .join(" ")
    .trim();
  return (
...
const ClapCount = ({ style: userStyle = {}, className }) => {
  const classNames = [styled.count, className].join(" ").trim();
...
const CountTotal = ({ style: userStyle = {}, className }) => {
  const classNames = [styled.total, className].join(" ").trim();
...
// Because everything in javascript is Object we can do that.
Clap.Icon = ClapIcon;
Clap.Count = ClapCount;
Clap.Total = CountTotal;
...
const Usage = () => {
  ...
  return (
    <div>
      <Clap className={userCostumStyle.clap} onClap={handleClap}>
        <Clap.Icon className={userCostumStyle.icon} />
        <Clap.Count className={userCostumStyle.count} />
        <Clap.Total className={userCostumStyle.total} />
      </Clap>
      <div>{`You have claped ${countt} time!`}</div>
    </div>
  );
};
