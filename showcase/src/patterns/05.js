...
const Clap = ({
  ...
  useEffect(() => {
    if (!compnentJustMounted.current && !isControlled) {
  ...
  const handleClapClick = () => {
    animationTimeLine.replay(); // handle animation
    isControlled
      ? onClap()
...
const Usage = () => {
  ...
  const handleClap = () => {
    setState((prev) => ({
      ...
      countTotal: Math.min(prev.count < MAX_USER_CLAP ? prev.countTotal + 1 : prev.countTotal),
      isClicked: true
    }));
  };
  return (
